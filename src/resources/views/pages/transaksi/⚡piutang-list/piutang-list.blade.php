<div class="p-6">
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold">Daftar Piutang</h1>
            <p class="text-sm text-gray-500">Piutang dari customer yang belum lunas.</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow p-4 mb-5">
        <div class="grid grid-cols-12 gap-4 items-end">
            <div class="col-span-12 md:col-span-3">
                <label class="block text-sm font-medium mb-2">Cari Customer/Nomor</label>
                <input type="text" wire:model.live.debounce.300ms="searchKeyword" placeholder="Nama atau nomor piutang..." class="w-full border rounded-lg px-3 py-2">
            </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                <thead class="bg-gray-100">
                    <tr>
                        <th class="px-4 py-3 text-left">No. Piutang</th>
                ...
                <tbody>
                    @forelse($piutangs as $piutang)
                        <tr class="border-t">
                            <td class="px-4 py-3 font-mono text-xs">{{ $piutang->nomor_piutang }}</td>
                            <td class="px-4 py-3">{{ $piutang->customer }}</td>
                            <td class="px-4 py-3">{{ $piutang->cabang->nama_cabang ?? '-' }}</td>
                            <td class="px-4 py-3">{{ \Carbon\Carbon::parse($piutang->tanggal)->format('d/m/Y') }}</td>
                            <td class="px-4 py-3 text-right">{{ number_format($piutang->jumlah, 0, ',', '.') }}</td>
                            <td class="px-4 py-3 text-right font-semibold {{ (float) $piutang->sisa > 0 ? 'text-red-600' : 'text-green-600' }}">
                                {{ number_format($piutang->sisa, 0, ',', '.') }}
                            </td>
                            <td class="px-4 py-3 text-center">
                                <span class="px-2 py-1 rounded text-xs font-semibold {{ $piutang->status === 'LUNAS' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">
                                    {{ $piutang->status === 'LUNAS' ? 'LUNAS' : 'BELUM LUNAS' }}
                                </span>
                            </td>
                            <td class="px-4 py-3 text-center">
                                @if($piutang->status === 'BELUM_LUNAS')
                                    <button wire:click="openPelunasanModal({{ $piutang->id }})" class="px-3 py-1 bg-blue-600 text-white rounded hover:bg-blue-700 text-xs">
                                        Bayar
                                    </button>
                                @endif
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="text-center py-8 text-gray-500">Data piutang belum tersedia.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    @if($showPelunasanModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" wire:click.self="closePelunasanModal()">
            <div class="w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
                <h2 class="text-lg font-semibold mb-4">Pelunasan Piutang</h2>
